...
const csvUrlnode = "./rawData.csv"
    ...
    function NodeInGraph(props) {
        ...
        return <circle key={node.id} cx={node.x} cy={node.y} r={radius} fill={color} name={node.name} pop={node.pop}></circle>
    }
    function Link(props) {
        const {link, lineWidth} = props;
        return <line key={link.index} x1={link.source.x} y1={link.source.y} x2={link.target.x} y2={link.target.y} stroke={"gray"} strokeWidth={lineWidth} name={link.source.name} name2={link.target.name} anum={link.affixNum}></line>
    }
    ...
    function Graph(props) {
        ...
        return <g transform={`translate(${x}, ${y})`}>
            {links.map(d=><Link key={d.Language_index} link={d} lineWidth={lineWidth(d.affixNum)}/>)}
            {nodes.map(d=><NodeInGraph key={d.Language_index} node={d} radius={radius(d.value)} color={color(d.pop)}/>)}
            </g>
